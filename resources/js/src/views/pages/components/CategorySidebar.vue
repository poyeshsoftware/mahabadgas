<template>
    <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary"
        ...
        <div class="mt-6 flex items-center justify-between px-6">
            <h4>{{ this.dataId === null ? $t('Add_New_Category') : $t('Update_Category') }}</h4>
        ...
        <VuePerfectScrollbar class="scroll-area--data-list-add-new" :key="$vs.rtl">
            ...
            <div class="p-6">
                <vs-input :label=" $t('Category_Name') " v-model="dataName" class="mt-5 w-full" name="category_name"
                          v-validate="'required'"/>
                ...
                this.$validator.validateAll().then(result => {
                    if (result) {
                        ...
                        if (this.dataId !== null && this.dataId >= 0) {
                            this.$store.dispatch("dataList/updateCategory", obj).catch(err => {
                                console.error(err)
                            })
                        } else {
                            delete obj.id
