<template>
    <div id="data-list-list-view" class="data-list-container">
        ...
        <vs-table
            ...
            <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
                ...
                <div class="flex flex-wrap-reverse items-center data-list-btn-container">
                    ...
                    <div
                        ...
                        <span class="ml-2 text-base text-primary">{{ $t('Add_New_Factor') }}</span>
                    </div>
                </div>
                ...
                <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
                    <div
                        ...
                        <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ factors.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : factors.length }} {{ $t('of') }} {{ queriedItems }}</span>
            ...
            <template slot="thead">
                <vs-th sort-key="name">{{$t('Factor_Title')}}</vs-th>
                <vs-th sort-key="name">{{$t('Factor_Related_Categories')}}</vs-th>
                <vs-th sort-key="name">{{$t('Factor_Related_Vehicles')}}</vs-th>
                <vs-th sort-key="name">{{$t('Factor_Related_Drivers')}}</vs-th>
                <vs-th sort-key="name">{{$t('Factor_Date')}}</vs-th>
            ...
            <template slot-scope="{data}">
                ...
                <vs-tr v-for="(tr, indextr) in data" :data="tr" :key="indextr">
                    ...
                    <vs-td>
                        <vs-chip v-for="(item,index) in tr.categories"
                                 :key="index" color="dark text-base">
                            {{ item.name }}
                        </vs-chip>
                    </vs-td>
                    <vs-td>
                        <vs-chip v-for="(item,index) in tr.vehicles"
                                 :key="index" color="dark text-base">
                            {{ item.name }}
                        </vs-chip>
                    </vs-td>
                    <vs-td>
                        <vs-chip v-for="(item,index) in tr.drivers"
                                 :key="index" color="dark text-base">
                            {{ item.name }}
                        </vs-chip>
                    </vs-td>
    ...
    export default {
        ...
        methods: {
            ...
            async acceptDelete(tr) {
                await this.$store.dispatch("dataList/removeFactor", tr.id).catch(err => {
                    console.error(err);
                    return 0;
                })
                this.$vs.notify({
                    ...
                    text: this.$t('Factor_Has_Been_Removed_Msg', [tr.title])
                })
            },
            editData(data) {
